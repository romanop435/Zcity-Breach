...
function SWEP:ThinkAdd()
	if CLIENT and self:GetWM() and not isbool(self:GetWM()) and isstring(self.FakeBodyGroups) then
		if self:HasAttachment("magwell", "mag1") then
			self:GetWM():SetBodyGroups("0200")
			self.settedGroups = true
			self.AnimList = {
				["idle"] = "idle",
				["reload"] = "reload_33",
				["reload_empty"] = "reload_empty_33",
			}
		else
...
SWEP.Primary.Sound = {"weapons/arccw_ud/glock/fire.ogg", 75, 90, 100}
SWEP.SupressedSound = {"weapons/arccw_ud/glock/fire-sup-01.ogg", 55, 90, 100}
...
SWEP.lengthSub = 25
SWEP.DistSound = "weapons/arccw_ud/glock/fire_dist.ogg"
