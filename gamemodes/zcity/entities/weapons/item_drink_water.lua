...
if CLIENT then
    ...
    SWEP.IconOverride = "nextoren/gui/new_icons/water.png"
    SWEP.Desc = "Освежает и восстанавливает выносливость."
...
function SWEP:OnRemove()
    ...
    timer.Remove("WaterPunch_" .. wepID)
    timer.Remove("WaterDrink_" .. wepID)
end
...
function SWEP:PrimaryAttack()
    ...
    timer.Create("WaterPunch_" .. wepID, 0.5, 1, function()
    ...
    timer.Create("WaterDrink_" .. wepID, 1.0, 1, function()
        ...
        if SERVER then
            ...
            if owner.organism then
                if owner.organism.stamina then
                    owner.organism.stamina[1] = math.min((owner.organism.stamina[1] or 0) + 40, owner.organism.stamina.max or 180)
                end
                
                
                owner.organism.satiety = math.min((owner.organism.satiety or 0) + 5, 100)
                owner.organism.hungry = math.max((owner.organism.hungry or 0) - 5, 0)
            end
        end
    end)
end
