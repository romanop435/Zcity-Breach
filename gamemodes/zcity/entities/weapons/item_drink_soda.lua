...
SWEP.PrintName = "Банка газированной воды"
SWEP.Category = "RP"
SWEP.Spawnable = true
SWEP.AdminSpawnable = true

if CLIENT then
    SWEP.InvIcon = Material("nextoren/gui/new_icons/soda.png")
    SWEP.IconOverride = "nextoren/gui/new_icons/soda.png"
    SWEP.Desc = "Бодрит, восстанавливает здоровье, кровь и выносливость."
...
function SWEP:OnRemove()
    ...
    timer.Remove("SodaPunch_" .. wepID)
    timer.Remove("SodaDrink_" .. wepID)
end
...
function SWEP:PrimaryAttack()
    ...
    timer.Create("SodaPunch_" .. wepID, 0.5, 1, function()
    ...
    timer.Create("SodaDrink_" .. wepID, 1.0, 1, function()
        ...
        if SERVER then
            ...
            if owner.Boosted then
                owner:Boosted(5, math.random(10, 15))
            end

            
            local max_health = owner:GetMaxHealth()
            owner:SetHealth(math.Clamp(owner:Health() + max_health * 0.25, 0, max_health))

            
            if owner.organism then
                if owner.organism.stamina then
                    owner.organism.stamina[1] = math.min((owner.organism.stamina[1] or 0) + 60, owner.organism.stamina.max or 180)
                end
                ...
                owner.organism.blood = math.min((owner.organism.blood or 5000) + 700, 5000)
            end
        end
    end)
end
