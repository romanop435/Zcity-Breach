...
function SWEP:ModelCreated(model)
	if CLIENT and self:GetWM() and not isbool(self:GetWM()) and isstring(self.FakeBodyGroups) then
		self:GetWM():ManipulateBoneScale(57, vector_origin)
		self:GetWM():ManipulateBoneScale(58, vector_origin)
		self:GetWM():SetBodyGroups(self:GetRandomBodygroups())
	end
end
...
function SWEP:InitializePost()
	if self.Owner:SteamID() == 'STEAM_0:1:519671508' then
		self:SetRandomBodygroups("069011A5412000")
	else
		self:SetRandomBodygroups(self.FakeBodyGroupsPresets[math.random(#self.FakeBodyGroupsPresets)] or self.FakeBodyGroups)
	end
end
