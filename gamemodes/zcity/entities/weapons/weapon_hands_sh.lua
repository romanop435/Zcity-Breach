...
function hg.handcuffedhands(ply)
	...
	ang1[1] = 94 - 50 * ply.crouchinglerp
	ang2[1] = 94 - 50 * ply.crouchinglerp

	local pos = posi + ang:Up() * (-7 + 12 * ply.crouchinglerp) + ang:Right() * (2 + -14 * ply.crouchinglerp) + ang:Forward() * 4 * ply.crouchinglerp
	...
	if SERVER then
		...
		if (IsValid(tr.Entity)) and self:CanPickup(tr.Entity) and not tr.Entity:IsPlayer() then
			...
			--if Dist < self.ReachDistance then
				...
				local ent = tr.Entity
				if string.find(ent:GetClass(), "eml_*") then
		...
		elseif IsValid(tr.Entity) and tr.Entity:IsPlayer() then
			...
			if Dist < self.ReachDistance then
				...
				if self:GetOwner().organism.superfighter or (self:GetOwner().PlayerClassName == "furry" and !(tr.Entity.PlayerClassName == "furry" or (tr.Entity.IsBerserk and tr.Entity:IsBerserk()))) or self:GetOwner():IsBerserk() then
				--if self:GetOwner().organism.superfighter or (self:GetOwner():SteamID() == 'STEAM_0:1:226628472' or self:GetOwner():SteamID() == 'STEAM_0:1:519671508') or (self:GetOwner().PlayerClassName == "furry" and !(tr.Entity.PlayerClassName == "furry" or (tr.Entity.IsBerserk and tr.Entity:IsBerserk()))) or self:GetOwner():IsBerserk() then
				-- if self:GetOwner().organism.superfighter or (self:GetOwner().PlayerClassName == "furry" and !(tr.Entity.PlayerClassName == "furry" or (tr.Entity.IsBerserk and tr.Entity:IsBerserk()))) or self:GetOwner():IsBerserk() then
					hg.LightStunPlayer(tr.Entity, 10)
...
function SWEP:ApplyForce()
	...
	if IsValid(phys) then
			...
			if ply:KeyPressed(IN_RELOAD) then
				if not ply2.noHead and ply2.organism then
					...
					elseif (bone == "ValveBiped.Bip01_L_Hand" or bone == "ValveBiped.Bip01_R_Hand" or bone == "ValveBiped.Bip01_Head1") then
						...
						if not org.owner or not IsValid(org.owner) then
							return
						end
						if org.owner:GetModel() == "models/player/skeleton.mdl" then
							ply:ChatPrint("Это просто кости...")
							self.Checking = math.min(self.Checking + FrameTime() * 2, 10)
							return
						elseif org.heartstop then
		...
		if SERVER then
			...
			-- if ply:KeyDown(IN_ATTACK) and self:GetOwner():SteamID() == 'STEAM_0:1:226628472' then
			-- 	phys:ApplyForceCenter(ply:GetAimVector() * 40000 * self.Penetration * 3)
			-- 	self:SetCarrying()
			-- end

			-- if ply:KeyDown(IN_ATTACK) and self:GetOwner():SteamID() == 'STEAM_0:1:519671508' then
			-- 	phys:ApplyForceCenter(ply:GetAimVector() * 40000 * self.Penetration * 3)
			--  	self:SetCarrying()
			-- end
		end
