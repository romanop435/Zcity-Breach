...
function SWEP:Reload(time)
	...
	-- Ученый путается в патронах и затворе
	if self:GetOwner().GTeam and self:GetOwner():GTeam() == TEAM_SCI then
		self.StaminaReloadMul = self.StaminaReloadMul * 2 -- В 2 раза дольше
	end

	-- Обратите внимание: я изменил 1.5 на 3.0, иначе кап не пропустит долгую перезарядку
	self.StaminaReloadMul = math.Clamp(self.StaminaReloadMul, 0.65, 3.0)
