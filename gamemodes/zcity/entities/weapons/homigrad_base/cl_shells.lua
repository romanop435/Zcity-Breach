...
concommand.Add('clearshels', function(pl)
    if not pl:IsSuperAdmin() then return end
    for _, ent in ipairs(hg_shelles) do
        ent:Remove()
    end
    hg_shelles = {}
end)
...
function SetupTimer()
    timer.Create("ClearShells", 300, 0, function()
        ClearShells()
    end)
end
hook.Add("InitPostEntity", "CreateEntityClearShells", SetupTimer)
hook.Add("PostCleanupMap", "CreateEntityClearShells", SetupTimer)