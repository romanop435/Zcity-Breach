...
--
	...
	CreateMenu = function()
		...
		frame.First = function() end
		hg.attachmentsMenuPanel = frame
		frame:SetTitle("")
		frame:SetSize( 0, shizlib.hud.ScrH / 2 )
		frame:SetPos( ScrW() * .5 - frame:GetWide() * .5, shizlib.hud.ScrH / 2 - shizlib.hud.ScrH / 4 )
		frame:MakePopup()
		frame:SetKeyboardInputEnabled(false)

		frame:SetAlpha(0)
	
		frame:MoveTo( shizlib.hud.ScrW * .5 - shizlib.hud.ScrW / 6, shizlib.hud.ScrH / 2 - shizlib.hud.ScrH / 4, 1.2, 0, .3, function() end )
		frame:SizeTo( shizlib.hud.ScrW / 3, shizlib.hud.ScrH / 2, 1.2, 0, .3, function() end )
