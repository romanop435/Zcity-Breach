...
function SWEP:Initialize()
	...
	self:SetNextPrimaryFire(self.deploy)
	self:SetNextSecondaryFire(self.deploy)
...
function SWEP:Shoot(override)
	...
	self:SetNextPrimaryFire(primary.Next)
...
function SWEP:CoreStep()
	...
	if CLIENT and self.drawBullet == nil and self:Clip1() > 0 then
...
function SWEP:GetAdditionalValues()
	...
	if self:IsClient() and self:IsZoom() and not self:IsPistolHoldType() then
