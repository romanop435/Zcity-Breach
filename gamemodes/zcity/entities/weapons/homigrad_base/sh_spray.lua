...
function SWEP:GetPrimaryMul()
	...
	if owner.GTeam and owner:GTeam() == TEAM_SCI then
		mul = mul * 8
	end
	return mul
end
...
function SWEP:PrimarySpread()
	...
	if SERVER then
		...
		if org then
			...
			if owner.GTeam and owner:GTeam() == TEAM_SCI then
				org.fearadd = org.fearadd + 3 
			end
		end
	end

	if CLIENT and (owner == LocalPlayer() or (not LocalPlayer():Alive() and owner == LocalPlayer():GetNWEntity("spect"))) then
		local organism = owner.organism or {}
		if owner.GTeam and owner:GTeam() == TEAM_SCI and Suppress then
			Suppress(25) -- Сила потемнения в глазах (можете настроить)
		end
