...
SWEP.PrintName = "Белый Монстр"
SWEP.Category = "RP"
SWEP.Spawnable = true
SWEP.AdminSpawnable = true

if CLIENT then
    SWEP.InvIcon = Material("nextoren/gui/new_icons/wmonster.png")
    SWEP.IconOverride = "nextoren/gui/new_icons/wmonster.png"
    SWEP.Desc = "Знаменитый энергетик. Дает сильный прилив бодрости и выносливости."
    SWEP.BounceWeaponIcon = false
end

SWEP.HoldType = "slam"
SWEP.ViewModel = ""
SWEP.WorldModel = "models/cultist/items/wmonster/whitemonster.mdl"
SWEP.WorldModelReal = "models/cultist/items/wmonster/v_monster.mdl"
...
function SWEP:OnRemove()
    ...
    timer.Remove("MonsterPunch_" .. wepID)
    timer.Remove("MonsterDrink_" .. wepID)
end
...
function SWEP:PrimaryAttack()
    ...
    timer.Create("MonsterPunch_" .. wepID, 0.5, 1, function()
    ...
    timer.Create("MonsterDrink_" .. wepID, 1.0, 1, function()
        ...
        if SERVER then
            ...
            if owner.Boosted then
                owner:Boosted(1, math.random(9, 13))
            end

            
            if owner.organism then
                if owner.organism.stamina then
                    owner.organism.stamina[1] = math.min((owner.organism.stamina[1] or 0) + 80, owner.organism.stamina.max or 180)
                end
