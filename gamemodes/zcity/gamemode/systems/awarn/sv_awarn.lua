...
function awarn_kick(ply, message)
    if IsValid(ply) then ply:Kick(message) end
end

function awarn_ban(ply, time, message)
    if IsValid(ply) then ply:Ban(time, true, message) end
end
...
local module_path = GM.FolderName .. "/gamemode/systems/awarn/_modules/"
local files = file.Find(module_path .. "*.lua", "LUA")

for _, name in ipairs(files or {}) do
	ServerLog("AWarn: Loading module (" .. name .. ")\n")
