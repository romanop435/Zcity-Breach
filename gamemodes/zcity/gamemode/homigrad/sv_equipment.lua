...
local function protec(org, bone, dmg, dmgInfo, placement, armor, scale, scaleprot, punch, boneindex, dir, hit, ricochet)
	...
	local ply = org.owner
	local isJuggernaut = IsValid(ply) and ply:IsPlayer() and (JuggernautClasses[ply.PlayerClassName] or (ply.GetRoleName and string.find(ply:GetRoleName() or "", "Juggernaut")))

	local inflictor = dmgInfo:GetInflictor()
	local penetration = (inflictor.bullet and inflictor.bullet.Penetration) or 1
	
	local armorData = hg.armor[placement] and hg.armor[placement][armor]
	local baseProtection = armorData and armorData.protection or 10

	-- Джаггернауты получают огромный буст к броне, делая их неуязвимыми для мелкого/среднего калибра
	if isJuggernaut then
		baseProtection = baseProtection * 3
	end

	local prot = baseProtection - penetration
	...
	if prot < 0 then
		if isJuggernaut then
			-- Даже если пробило, джаггернаут танкует часть урона телом
	...
	-- Броня выдержала попадание
	dmgInfo:SetDamageType(DMG_CLUB)
	dmgInfo:SetDamageForce(dmgInfo:GetDamageForce() * 0.4)
	
	if isJuggernaut then
		dmgInfo:ScaleDamage(0.01) -- Убираем заброневой урон в ноль, чтобы не копился шок
		return 1 -- ВОЗВРАЩАЕМ 1: пуля полностью останавливается в броне и НЕ летит дальше в череп/мозг
	else
		dmgInfo:ScaleDamage(0.1) -- Обычным игрокам тоже снизили заброневой урон (было 0.2)
		return 1 -- Обычная броня теперь тоже на 100% останавливает пулю, если не пробита
	end
end
