...
--\\ BoneMatrix func
	local function setbonematrix(self, bone, matrix) end
...
--\\ other ents use our bullets
	...
	hook.Add("EntityFireBullets", "NPC_Boolets", function(ent, bullet)
		if not IsValid(ent) or bullet.NpcShoot then return end
		local class = ent:GetClass()
		local tbl = npcs[class]
		if tbl then
			if class == "npc_turret_floor" and IsValid(ent:GetEnemy()) and ent:GetEnemy():GetClass() == "npc_bullseye" and IsValid(ent:GetEnemy().rag) then
			...
			if bullet.AmmoType then
				local ammoId = game.GetAmmoID(bullet.AmmoType)
				local settings = hg.ammotypeshuy[bullet.AmmoType] and hg.ammotypeshuy[bullet.AmmoType].BulletSettings
				local baseDamage = game.GetAmmoPlayerDamage(ammoId)
				bullet.Damage = settings and settings.Damage or baseDamage
				bullet.Force = (settings and settings.Force or baseDamage) * (tbl.force or 1)
				bullet.Penetration = (settings and settings.Penetration or baseDamage) * (tbl.PenetrationMul or 1)
			end
			bullet.Filter = { ent }
			bullet.Attacker = ent
			bullet.penetrated = 0
