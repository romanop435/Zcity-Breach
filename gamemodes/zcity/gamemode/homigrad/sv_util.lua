...
hook.Add( "EntityRemoved", "SniperShitCleanup", function( ent )
	if ent:GetClass() ~= "npc_sniper" then return end
	for i = #snipers, 1, -1 do
		if snipers[i] == ent then
			table.remove( snipers, i )
			break
		end
	end
end )
...
hook.Remove("Player Think", "homigrad-dropholstered")
...
function hgWreckBuildings(blaster, pos, power, range, ignoreVisChecks) -- taken from JMod -- this so unstable shit, can crush your game
	...
	for i, prop in ipairs(allProps) do
		if not WreckBlacklist[prop:GetClass()] and hook.Run("hg_CanDestroyProp", prop, blaster, pos, power, range, ignoreVisChecks) ~= false and prop.ExplProof ~= true then
			local propPos = prop:LocalToWorld(prop:OBBCenter())
			local index = #positionProps + 1
			positionProps[index] = prop
			positionVectors[index] = propPos
			positions[#positions + 1] = propPos.x
			positions[#positions + 1] = propPos.y
			positions[#positions + 1] = propPos.z
		end
	end

	local native = BREACH.Native
	local distances = (native and #positions > 0) and native.DistanceSqrBatch(pos.x, pos.y, pos.z, positions) or nil

	for i, prop in ipairs(positionProps) do
		if IsValid(prop) then
			local physObj = prop:GetPhysicsObject()
			local propPos = positionVectors[i]
			local distSqr = distances and distances[i] or propPos:DistToSqr(pos)
			local DistFrac = 1 - math.sqrt(distSqr) / maxRange
...
function hgBlastDoors(blaster, pos, power, range, ignoreVisChecks) -- taken from JMod
	for k, door in ipairs(ents.FindInSphere(pos, 40 * power * (range or 1))) do
...
hook.Remove("PostEntityTakeDamage", "GlassShards")
