...
PLUGIN.BulletsList = PLUGIN.BulletsList or {}
PLUGIN.BulletsListCount = PLUGIN.BulletsListCount or 0
...
--\\MetaTable
	...
	function PLUGIN.Class_Bullet:Remove()
		...
		local listIndex = self._listIndex
		local lastIndex = PLUGIN.BulletsListCount
		if listIndex and lastIndex > 0 then
			local lastBullet = PLUGIN.BulletsList[lastIndex]
			if listIndex ~= lastIndex then
				PLUGIN.BulletsList[listIndex] = lastBullet
				lastBullet._listIndex = listIndex
			end
			PLUGIN.BulletsList[lastIndex] = nil
			PLUGIN.BulletsListCount = lastIndex - 1
			self._listIndex = nil
		end
...
--\\Creation
	function PLUGIN.CreateBullet(bullet)
		...
		PLUGIN.BulletsListCount = PLUGIN.BulletsListCount + 1
		bullet._listIndex = PLUGIN.BulletsListCount
		PLUGIN.BulletsList[bullet._listIndex] = bullet
...
--\\Hooks
	PLUGIN:AddHook("Think", function()
		local i = 1
		while i <= PLUGIN.BulletsListCount do
			local bullet = PLUGIN.BulletsList[i]
			bullet:Think()
			if PLUGIN.BulletsList[i] == bullet then
				i = i + 1
			end
		end
	end)
...
hook.Add("PostCleanupMap", "PhysBullets", function()
	for i = PLUGIN.BulletsListCount, 1, -1 do
		PLUGIN.BulletsList[i]:Die()
	end
end)
