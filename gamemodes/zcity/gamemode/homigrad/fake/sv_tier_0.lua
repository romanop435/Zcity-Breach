...
🐔("SetupPlayerVisibility", "fuckragdolls", function( ply )
	...
	for ent, tbl in pairs(queue) do--ему, наверное, больно
		...
		if queue[ent].count == (🎂.humans_cached_count or 0) then queue[ent] = nil continue end
...
function 🎂.Fake(ply, huyragdoll, no_freemove, force) end
local veczero = Vector(0,0,0)
...
🐔("PlayerDisconnected", "🎂-killniers", function(ply)
	if ply:Alive() then
		...
		if not ply:GetPos():WithinAABox(Vector(-3847, 3237, 211), Vector(-3216, 2354, -100)) then
			...
			if inventory and isfunction(inventory.GetItems) and isfunction(inventory.SetItem) then
				for k in pairs(inventory:GetItems() or {}) do
					inventory:SetItem(k, nil)
				end
			end

			if itemstore and itemstore.data and isfunction(itemstore.data.Save) then
