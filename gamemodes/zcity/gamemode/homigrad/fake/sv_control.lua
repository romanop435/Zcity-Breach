...
local function getFakePhys(ragdoll, physNumber)
	local cache = ragdoll._hgPhysCache
	if not cache then
		cache = {}
		ragdoll._hgPhysCache = cache
	end

	local phys = cache[physNumber]
	if not IsValid(phys) then
		phys = ragdoll:GetPhysicsObjectNum(realPhysNum(ragdoll, physNumber))
		cache[physNumber] = phys
	end
...
hook.Add("Fake", "Contorl", function(ply, ragdoll)
	...
	ragdoll._hgTorsoBone = ragdoll:LookupBone("ValveBiped.Bip01_Spine2")
	ragdoll._hgHeadBone = ragdoll:LookupBone("ValveBiped.Bip01_Head1")
	ragdoll._hgEyesAttachment = ragdoll:LookupAttachment("eyes")
	ragdoll._hgPhysCache = nil
end)
...
hg.humans_cached_count = #player_GetHumans()
hook.Add("PlayerInitialSpawn", "FakeHumanCount", function()
	timer.Simple(0, function() hg.humans_cached_count = #player_GetHumans() end)
end)
hook.Add("PlayerDisconnected", "FakeHumanCount", function()	hg.humans_cached_count = #player_GetHumans() - 1	end)
...
hook.Add("Think", "Fake", function()
	...
	for ply, ragdoll in pairs(hg.ragdollFake) do
		...
		local torso = ragdoll._hgTorsoBone
		if torso then
		...
		ragdoll.dtime = (sysNow - (ragdoll.lastCallTime or sysNow)) * timeScale
		ragdoll.lastCallTime = sysNow

		local mainPhys = ragdoll:GetPhysicsObject()
		local velocity = mainPhys:GetVelocity()
		local velocityLenSqr = velocity:LengthSqr()

		local org = ply.organism
		local wep = ply:GetActiveWeapon()

		local plyPos = ply:GetPos()
		local tr = ragdoll._hgFakeGroundTrace
		if not tr then
			tr = {filter = {ply, ragdoll}}
			ragdoll._hgFakeGroundTrace = tr
		end
		tr.start = plyPos
		tr.endpos = plyPos - vector_up * 10
		...
		elseif ply:Alive() then			
			local pos = ragdoll:GetBoneMatrix(ragdoll._hgHeadBone):GetTranslation()		
		...
		local att = ragdoll:GetAttachment(ragdoll._hgEyesAttachment)
		...
		spine = getFakePhys(ragdoll, 1)
		rhand = getFakePhys(ragdoll, 7)
		lhand = getFakePhys(ragdoll, 5)
		ang = spine:GetAngles()
			...
			if tracehuy.Hit then
				...
				if ragdoll:LookupBone(wound[4]) then
					...
					if not ply:KeyDown(IN_USE) then
						shadowControl(ragdoll, 10, 0.001, nil, nil, nil, getFakePhys(ragdoll, 8):GetPos(), 40, 10)
						shadowControl(ragdoll, 1, 0.001, nil, nil, nil, getFakePhys(ragdoll, 8):GetPos(), 00, 10)
						shadowControl(ragdoll, 2, 0.001, nil, nil, nil, getFakePhys(ragdoll, 8):GetPos(), 0, 10)
						shadowControl(ragdoll, 3, 0.001, nil, nil, nil, getFakePhys(ragdoll, 8):GetPos(), 20, 10)
		...
		if not wep.RagdollFunc then
			...
			if !IsValid(ragdoll.ConsLH) and (ply:KeyDown(IN_ATTACK) and !ishgweapon(wep)) or ((ishgweapon(wep) or wep.ismelee2) and (ply:KeyDown(IN_USE) or ply:KeyDown(IN_ATTACK2))) then// || ply:InVehicle() then
				if org.canmove then
					//if !ply:InVehicle() then
						...
						shadowControl(ragdoll, 5, 0.001, ang2, forceArm * 2, forceArm_dump, getFakePhys(ragdoll, 5):GetPos() + ang2:Forward() * 15 + ((velocityLenSqr > 22500 and velocity / 224) or vector_zero), 500, 50)
						if ply:WaterLevel() == 1 then shadowControl(ragdoll, 1, 0.001, nil, nil, nil, getFakePhys(ragdoll, 5):GetPos(), forceArmWater, forceArmWater_dump) end
					/*else
						ang2:Set(angles)
			...
			if forward then
				if IsValid(ragdoll.ConsRH) then
					...
					local torso = getFakePhys(ragdoll, 1)
			...
			if !IsValid(ragdoll.ConsRH) and ply:KeyDown(IN_ATTACK2) or ((ishgweapon(wep) or wep.ismelee2) and ply:KeyDown(IN_USE)) then// || ply:InVehicle() then
				if org.canmove then
					...
					//if !ply:InVehicle() then
						...
						shadowControl(ragdoll, 7, 0.001, ang2, forceArm * 2, forceArm_dump, getFakePhys(ragdoll, 7):GetPos() + ang2:Forward() * 15 + ((velocityLenSqr > 22500 and velocity / 224) or vector_zero), ishgweapon(wep) and 500 or 500, ishgweapon(wep) and 50 or 50)
			...
			if ply:KeyDown(IN_ATTACK2) and org.canmove then
				if wep.RagdollFunc then
					wep:RagdollFunc(getFakePhys(ragdoll, 7):GetPos() + angles:Forward() * 15 + ((velocityLenSqr > 22500 and velocity / 224) or vector_zero), angles, ragdoll)
				end
			end
		end
		-- Zavtra yje
		...
		if ply:KeyDown(IN_MOVELEFT) and ragdoll:IsOnFire() and not inmove and !ply:InVehicle() then
			if org.canmove then
				...
				local head = ragdoll:GetPhysicsObject(ragdoll:TranslateBoneToPhysBone(ragdoll._hgHeadBone))
		...
		if ply:KeyDown(IN_DUCK) and !ply:InVehicle() then
			if org.canmove and org.spine1 < hg.organism.fake_spine1 then
				...
				local rleg = getFakePhys(ragdoll, 13)
				local lleg = getFakePhys(ragdoll, 14)
