...
function PANEL:Paint(w,h)
    ...
    surface.SetDrawColor(ColorAlpha(hg.VGUI.MainColor, 20))
...
function PANEL:PostInit()
    ...
    function savePresetBtn:DoClick()
        ...
        if presetName == "" or #presetName < 2 then
            ...
            notification.AddLegacy("Введите название пресета (минимум 2 символа)", NOTIFY_ERROR, 3)
            return
        end
    ...
    function deletePresetBtn:DoClick()
        ...
        if presetName == "" then
            surface.PlaySound("buttons/button10.wav")
            notification.AddLegacy("Введите название пресета для удаления", NOTIFY_ERROR, 3)
            return
        end
        
        if DeletePreset(presetName) then
            surface.PlaySound("buttons/button15.wav")
            notification.AddLegacy("Пресет '" .. presetName .. "' удалён!", NOTIFY_HINT, 3)
    ...
    presetNameEntry:SetPlaceholderText("Название пресета...")
    ...
    function hatSelector:DoClick()
        ...
        hatSelectMenu = CreateStyledAccessoryMenu(nil, "Выбор шляпы")
    ...
    function faceSelector:DoClick()
        ...
        faceSelectorMenu = CreateStyledAccessoryMenu(nil, "Выбор аксессуара для лица")
    ...
    function bodySelector:DoClick()
        ...
        bodySelectorMenu = CreateStyledAccessoryMenu(nil, "Выбор аксессуара для тела")
