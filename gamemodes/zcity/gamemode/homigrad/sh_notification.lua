	...
	local function NotificationsDraw()
		...
		if tbl and istable(tbl) and not table.IsEmpty(tbl) then
			...
			if (time + time_to_read + wait > time_spent) then
				...
				local is_robot = lply.GTeam and lply:GTeam() == TEAM_AR

				if click != oldclick and not last_message then
					if is_robot then
						sound.Play("robot_peepsnd", render.GetViewSetup().origin - vector_up * 10)
					else
				...
				local font = is_robot and "ImpactSmallest" or hg.notificationFont
...
--ply:Notify(args[2], 6, nil, 0, nil, Color(255, 255, 255))