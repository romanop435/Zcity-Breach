...
module[2] = function(owner, org, timeValue)
	...
	if owner:IsPlayer() then
		...
		velLen = max(min(owner:GetVelocity():Length(), org.moveMaxSpeed), 0) / (owner:GetRunSpeed() / .8)-- / ((IsValid(wep) and wep ~= NULL and wep:GetClass() == "weapon_hands_sh" and owner:KeyDown(IN_WALK)) and 1.3 or 0.58))
		--print(velLen)
	...
	-- БЕСКОНЕЧНАЯ ВЫНОСЛИВОСТЬ У РОБОТОВ
	if owner.GTeam and owner:GTeam() == TEAM_AR then
		stamina.sub = 0
		stamina[1] = stamina.max
	else
		stamina[1] = max(stamina[1] - stamina.sub * timeValue * 21, 0)
