...
local function applySpasm(rag, stype)
	...
	local now = CurTime()
	rag.spasmEnd, rag.spasmStart = now + dur, now
...
local function processExtend(rag, fade, curTime)
...
local function processRigor(rag, fade, curTime)
...
local function processFlexion(rag, fade, curTime)
...
local function applyFencingToPlayer(ply, org)
	...
	org.fencing = true
	org.fencingEnd = (org.curTime or CurTime()) + dur
	org.fencingDur = dur
...
local function processFencing(rag, fade, curTime)
...
hook.Add("Org Think", "BrainfuckThink", function(owner)
	...
	if org.fencing and org.fencingEnd then
		...
		if IsValid(rag) then
			if curTime > org.fencingEnd then
				...
				processFencing(rag, fade, curTime)
			end
		end
	end
	...
	if IsValid(deathRag) and deathRag.spasm and deathRag.spasmEnd then
		if curTime > deathRag.spasmEnd then
			...
			if stype == "extend" then processExtend(deathRag, fade, curTime)
			elseif stype == "rigor" then processRigor(deathRag, fade, curTime)
			elseif stype == "flexion" then processFlexion(deathRag, fade, curTime) end
		end
	end
end)
