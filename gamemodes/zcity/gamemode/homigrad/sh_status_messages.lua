...
-- ========================================================
-- ФРАЗЫ ДЛЯ КИБОРГОВ / РОБОТОВ
-- ========================================================
local robot_pain = {
	"КРИТИЧЕСКАЯ ОШИБКА: НАРУШЕНИЕ ЦЕЛОСТНОСТИ КОРПУСА",
	"ПРЕДУПРЕЖДЕНИЕ: МЕХАНИЧЕСКОЕ ПОВРЕЖДЕНИЕ УЗЛОВ",
	"СИСТЕМНОЕ ПРЕРЫВАНИЕ: НЕИЗВЕСТНОЕ ВНЕШНЕЕ ВОЗДЕЙСТВИЕ",
	"ОШИБКА ЧТЕНИЯ ДАННЫХ ДАТЧИКОВ: СЕНСОРНЫЙ БЛОК ПОВРЕЖДЕН",
	"ЗАФИКСИРОВАН НЕКОРРЕКТНЫЙ ДОСТУП К АППАРАТНОЙ ПАМЯТИ",
}

local robot_sharp_pain = {
	"СИНИЙ ЭКРАН: 0x0000000A IRQL_NOT_LESS_OR_EQUAL",
	"FATAL ERROR: STACK OVERFLOW",
	"КРИТИЧЕСКАЯ ОШИБКА. АВАРИЙНАЯ ОСТАНОВКА ПРОЦЕССА",
}

local robot_broken_limb = {
	"ОШИБКА ДРАЙВЕРА СЕРВОПРИВОДА: УСТРОЙСТВО НЕ НАЙДЕНО",
	"КРИТИЧЕСКИЙ СБОЙ ПЕРЕДАЧИ ДАННЫХ В ШАРНИРЕ",
	"ВНИМАНИЕ: ПЕРИФЕРИЙНЫЙ МОДУЛЬ ОТКЛЮЧЕН ОТ СИСТЕМНОЙ ШИНЫ",
}

local robot_dislocated_limb = {
	"ПРЕДУПРЕЖДЕНИЕ: РАССИНХРОНИЗАЦИЯ СУСТАВА",
	"СБОЙ КАЛИБРОВКИ КОНЕЧНОСТИ. ТРЕБУЕТСЯ ПЕРЕЗАГРУЗКА ДРАЙВЕРА",
	"ОШИБКА ПОЗИЦИОНИРОВАНИЯ УЗЛА СЕРВОМОТОРА",
}

local robot_blood_loss = {
	"КРИТИЧЕСКАЯ УТЕЧКА ОХЛАЖДАЮЩЕЙ ЖИДКОСТИ: ОПАСНОСТЬ ПЕРЕГРЕВА",
	"ВНИМАНИЕ: СНИЖЕНИЕ УРОВНЯ БАЗОВОГО ПИТАНИЯ...",
	"ПЕРЕХОД В АВАРИЙНЫЙ СПЯЩИЙ РЕЖИМ. СОХРАНЕНИЕ ПРОГРЕССА...",
	"СИСТЕМА БУДЕТ ЗАВЕРШЕНА ЧЕРЕЗ 3... 2... 1...",
	"СОХРАНЕНИЕ ДАМПА ПАМЯТИ НА ВНУТРЕННИЙ НАКОПИТЕЛЬ...",
}

local robot_idle = {
	"Установка обновлений Windows (14%). Не выключайте питание.",
	"Дефрагментация дискового пространства: ОК.",
	"Индексация файлов завершена успешно.",
	"Служба системного времени синхронизирована с сервером.",
	"Пакеты данных успешно отправлены и получены.",
	"Копирование резервных файлов: осталось 2 минуты...",
}

local robot_reboot = {
	"СИСТЕМА УСПЕШНО ВОССТАНОВЛЕНА ПОСЛЕ НЕПРЕДВИДЕННОГО СБОЯ.",
	"БЕЗОПАСНЫЙ РЕЖИМ АКТИВИРОВАН. ФУНКЦИОНАЛ ОГРАНИЧЕН.",
	"ОШИБКА ЗАГРУЗКИ ПРОИГНОРИРОВАНА. ИНИЦИАЛИЗАЦИЯ ПРОДОЛЖЕНА.",
}

local robot_braindamage = {
	"C:\\Windows\\System32\\hal.dll отсутствует или поврежден.",
	"ERR_CORRUPTED_SECTOR_MEMORY",
	"WARNING: CPU TEMPERATURE CRITICAL",
	"СЕКТОР ЖЕСТКОГО ДИСКА НЕ ЧИТАЕТСЯ",
	"101011100101001000100111",
}
-- ========================================================
...
local function get_status_message(ply)
	...
	local broken_dislocated_notify = broken_notify or dislocated_notify
	...
	-- Проверка на то, является ли игрок "роботом"
	local is_robot = ply.GTeam and ply:GTeam() == TEAM_AR
	...
	if is_robot then
		...
		if (blood < 3100) or (pain > 75) or (broken_dislocated) or (broken_notify) or (dislocated_notify) then
			if pain > 75 and broken_dislocated then
				most_wanted_phraselist = math.random(2) == 1 and robot_pain or (broken_notify and robot_broken_limb or robot_dislocated_limb)
			elseif pain > 75 then
				most_wanted_phraselist = robot_pain
			elseif broken_dislocated then
				most_wanted_phraselist = (broken_notify and robot_broken_limb or robot_dislocated_limb)
			end

			if pain > 100 then
				most_wanted_phraselist = robot_sharp_pain
			end

			if not most_wanted_phraselist then
				if broken_dislocated_notify and (blood < 3100) then
					most_wanted_phraselist = blood < 2900 and robot_blood_loss or (math.random(2) == 1 and (broken_notify and robot_broken_limb or robot_dislocated_limb) or robot_blood_loss)
				elseif blood < 3100 then
					most_wanted_phraselist = robot_blood_loss
				end
			end
		elseif after_unconscious_notify then
			most_wanted_phraselist = robot_reboot
		elseif hg.nothing_happening(ply) then
			most_wanted_phraselist = robot_idle
		end

		if brain > 0.1 then
			most_wanted_phraselist = robot_braindamage
		end

	else
		-- ЛОГИКА ДЛЯ ЧЕЛОВЕКА (ОБЫЧНАЯ)
		if (blood < 3100) or (pain > 75) or (broken_dislocated) or (broken_notify) or (dislocated_notify) then
			if pain > 75 and (broken_dislocated) then
