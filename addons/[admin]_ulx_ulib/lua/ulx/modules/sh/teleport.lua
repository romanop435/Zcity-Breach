...
function ulx.adminzone( calling_ply, target )

	target.ulx_prevpos = target:GetPos()
	target.ulx_prevang = target:EyeAngles()
	target:SetPos(Vector(math.random(-1826, -1994), math.random(-5950, -6188), 14963.031250))

end
local adminzone = ulx.command( CATEGORY_NAME, "ulx adminzone", ulx.adminzone, "!adminzone" )
adminzone:addParam{ type=ULib.cmds.PlayerArg }
adminzone:defaultAccess( ULib.ACCESS_ADMIN )
adminzone:help( "Brings target(s) to you." )
...
function ulx.bring( calling_ply, target_plys )
  ...
  target_plys.ulx_prevpos = target_plys:GetPos()
	target_plys.ulx_prevang = target_plys:EyeAngles()
  target_plys:SetPos(calling_ply:GetPos())
...
function ulx.goto( calling_ply, target_ply )
	...
	if calling_ply:GTeam() == TEAM_SPEC then
		newpos = target_ply:GetPos()
	end
...
function ulx.retrn( calling_ply, target_plys )
	...
	ulx.fancyLogAdmin( calling_ply, "#A returned #T to their original position", target_plys )
end
local retrn = ulx.command( CATEGORY_NAME, "ulx return", ulx.retrn, "!return" )
retrn:addParam{ type=ULib.cmds.PlayersArg, ULib.cmds.optional }
