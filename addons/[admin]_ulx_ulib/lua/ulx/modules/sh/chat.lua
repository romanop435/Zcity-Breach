...
function ulx.discord( calling_ply )
	...
	BREACH.SendClientEvent(calling_ply, "open_url", {url = "https://discord.gg/4KmXXWcZFp"})
...
function ulx.asay( calling_ply, message )
	...
	if !admins then
		...
		BREACH.SendClientEvent(calling_ply, "open_url", {url = "https://discord.com/channels/985856355216789554/1405839318333001768"})
		return
	end
	...
	for _, admin in pairs(player.GetHumans()) do
		if admin:IsAdmin() or admin == calling_ply then
			if calling_ply:SteamID64() == "76561198966614836" then
				...
				BREACH.SendClientEvent(admin, "flash_window")
				BREACH.SendClientEvent(admin, "play_sound", {sound = "nextoren/gui/main_menu/confirm.wav"})
