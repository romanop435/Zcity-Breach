...
function bans.init()
	...
	xgui.addDataType( "bans", function() return { count=table.Count( BREACH.Punishment.Bans ) } end, "xgui_managebans", 30, 20 )
	...
	function bans.updateBan( ply, args )
		...
		if not BREACH.Punishment.Bans[steamID] then
		...
		if name == "" then
			name = nil
			BREACH.Punishment.Bans[steamID].name = nil
		end

		if bantime ~= 0 then
			if (BREACH.Punishment.Bans[steamID].time + bantime*60) <= os.time() then --New ban time makes the ban expired
				ULib.unban( steamID, ply )
				return
			end
			bantime = bantime - (os.time() - BREACH.Punishment.Bans[steamID].time)/60
		end
		ULib.addBan( steamID, bantime, reason, name, ply )
	end
	...
	function bans.getSortTable( sortType )
		...
		if sortType == 1 then
			...
			if next( xgui.bansbyname ) == nil then
				for k, v in pairs( BREACH.Punishment.Bans ) do
	...
	function bans.sendBansToUser( ply, args )
		...
		if table.Count(BREACH.Punishment.Bans) <= 0 then return end

		for i = startValue, endValue, ascending and -1 or 1 do
			local steamID = sortTable[i][1]
			local bandata = BREACH.Punishment.Bans[steamID]
	...
	function bans.unbanTimer()
		...
		for ID, data in pairs( BREACH.Punishment.Bans ) do
