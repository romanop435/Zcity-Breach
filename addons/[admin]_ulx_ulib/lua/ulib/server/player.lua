...
function ULib.getBanMessage( steamid, banData, templateMessage )
	banData = banData or BREACH.Punishment.Bans[ steamid ]
...
local function checkBan( steamid64, ip, password, clpassword, name )
	...
	if banData.unban != 0 and banData.unban < os.time() then  -- Not banned too
		BREACH.Punishment.Bans[ steamid ] = nil
		return
	end
...
function ULib.addBan( steamid, time, reason, name, admin )
	...
	if admin then
		admin_name = "(Console)"
		if !isstring(admin) and admin:IsValid() then
	...
	BREACH.Punishment:Ban(steamid, admin, time * 60, reason)
...
function ULib.unban( steamid, admin, reason )
	...
	--ULib.fileWrite( ULib.BANS_FILE, ULib.makeKeyValues( ULib.bans ) )
	BREACH.Punishment:Unban(steamid, admin, reason)
...
function ULib.spawn( player, bool )
	...
	if bool and player.ULibSpawnInfo then
		...
		player:SetMaxHealth( t.maxhealth )
